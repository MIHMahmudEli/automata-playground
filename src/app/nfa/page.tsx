...
export default function NFAWorkspace() {
  ...
  const exportPNG = useCallback(() => {
    if (!visualizerRef.current?.cy) { showMessage('Generate an NFA first!', 'error'); return; }
  ...
  const loadExample = useCallback((name: string) => {
    const examples: Record<string, any> = {
      endsWith01: { states: 'q0, q1, q2', alphabet: '0, 1', start: 'q0', accept: 'q2', transitions: 'q0,0,q0\nq0,1,q0\nq0,0,q1\nq1,1,q2' },
      contains00: { states: 'q0, q1, q2', alphabet: '0, 1', start: 'q0', accept: 'q2', transitions: 'q0,0,q1\nq0,1,q0\nq1,0,q2\nq1,1,q0\nq2,0,q2\nq2,1,q2' },
      startsOrEndsWith0: { states: 'q0, q1, q2, q3', alphabet: '0, 1', start: 'q0', accept: 'q1, q2', transitions: 'q0,0,q1\nq0,1,q3\nq1,0,q1\nq1,1,q1\nq3,0,q2\nq3,1,q3\nq2,0,q2\nq2,1,q2' },
    };
    const ex = examples[name];
    if (ex) {
      setStates(ex.states); setAlphabet(ex.alphabet); setStartState(ex.start);
      setAcceptStates(ex.accept); setTransitions(ex.transitions);
  ...
  return (
    <>
      <WorkspaceNavbar title="NFA Workspace" subtitle="Non-deterministic Finite Automaton" />
      <div className="pt-20 sm:pt-24 pb-10 sm:pb-12 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-8 sm:mb-12">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4">
              <span className="gradient-text">Non-deterministic Finite Automaton</span>
            </h1>
            <p className="text-gray-300 text-base sm:text-lg max-w-3xl mx-auto px-2 sm:px-0">Create, visualize, and test NFAs with our interactive workspace</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-6">
              <div className="glass-card rounded-2xl p-6 border-2 border-purple-500/30 overflow-hidden relative group">
                ...
                <div className="relative">
                  <input type="text" className="input-field w-full pl-4 pr-12 py-3 rounded-xl text-sm" placeholder="e.g. 'NFA that ends with 01'" value={aiQuestion} onChange={e => setAiQuestion(e.target.value)} onKeyDown={e => e.key === 'Enter' && askAI()} />
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button className="text-[10px] text-purple-300 hover:text-white transition" onClick={() => setAiQuestion('Ends with 01')}>&quot;Ends with 01&quot;</button>
                  <button className="text-[10px] text-purple-300 hover:text-white transition" onClick={() => setAiQuestion('Contains 00')}>&quot;Contains 00&quot;</button>
                  <button className="text-[10px] text-purple-300 hover:text-white transition" onClick={() => setAiQuestion('Starts or ends with 0')}>&quot;Starts or ends with 0&quot;</button>
                </div>
                {aiLoading && (
              ...
              <div className="glass-card rounded-2xl p-6">
                <h2 className="text-2xl font-bold text-white mb-6 flex items-center">
                  <i className="fas fa-cog text-purple-400 mr-3" /> NFA Configuration
                </h2>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea className="input-field w-full px-4 py-3 rounded-lg font-mono text-sm resize-none" rows={6} placeholder="q0,0,q0&#10;q0,0,q1&#10;q1,1,q2&#10;q0,ε,q2" value={transitions} onChange={e => setTransitions(e.target.value)} />
                  </div>
                  <div className="flex gap-3">
                    <button className="btn-primary flex-1 px-6 py-3 text-white font-semibold rounded-lg" onClick={generateNFA}>
                      <i className="fas fa-play mr-2" />Generate NFA
                    </button>
                  ...
                  <div className="border-t border-white/10 pt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button className="px-3 py-1.5 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/30 transition text-xs" onClick={() => loadExample('endsWith01')}>Ends with &quot;01&quot;</button>
                      <button className="px-3 py-1.5 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/30 transition text-xs" onClick={() => loadExample('contains00')}>Contains &quot;00&quot;</button>
                      <button className="px-3 py-1.5 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/30 transition text-xs" onClick={() => loadExample('startsOrEndsWith0')}>Starts/Ends with 0</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="glass-card rounded-2xl p-6">
              ...
              <div className="bg-white/5 rounded-xl border border-white/10 relative h-[250px] sm:h-[350px] md:h-[400px]" id="nfa-graph-container" ref={containerRef}>
                {!generated && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      <div className="w-20 h-20 bg-purple-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
                        <i className="fas fa-code-branch text-purple-400 text-3xl" />
                      </div>
                      <p className="text-gray-400 mb-2">Your NFA will appear here</p>
                      <p className="text-gray-500 text-sm">Configure and click &quot;Generate NFA&quot;</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <div className="text-xs text-gray-500">
                  <i className="fas fa-info-circle mr-1" />Double-click nodes
                </div>
              </div>
            </div>
          </div>
