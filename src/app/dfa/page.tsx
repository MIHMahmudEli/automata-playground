...
export default function DFAWorkspace() {
  ...
  const doRender = useCallback((validationData: Record<string, unknown>) => {
    ...
    visualizerRef.current.renderAutomaton(validationData as any, 'DFA');
  }, []);

  const generateDFA = useCallback(() => {
    const input = { states, alphabet, startState, acceptStates, transitions };
    const validation = AutomataCore.validateDFA(input);
  ...
  const askAI = useCallback(async () => {
    ...
    setAiLoading(true);
    try {
      const result = await AutomataAI.generateFromQuestion(aiQuestion, 'DFA');
  ...
  const exportPNG = useCallback(() => {
    if (!visualizerRef.current?.cy) { showMessage('Generate a DFA first!', 'error'); return; }
  ...
  const loadExample = useCallback((name: string) => {
    const examples: Record<string, any> = {
      even: { states: 'q0, q1', alphabet: '0, 1', start: 'q0', accept: 'q0', transitions: 'q0,0,q0\nq0,1,q1\nq1,0,q0\nq1,1,q1' },
      contains101: { states: 'q0, q1, q2, q3', alphabet: '0, 1', start: 'q0', accept: 'q3', transitions: 'q0,0,q0\nq0,1,q1\nq1,0,q2\nq1,1,q1\nq2,0,q0\nq2,1,q3\nq3,0,q3\nq3,1,q3' },
      div3: { states: 'q0, q1, q2', alphabet: '0, 1', start: 'q0', accept: 'q0', transitions: 'q0,0,q0\nq0,1,q1\nq1,0,q2\nq1,1,q0\nq2,0,q1\nq2,1,q2' },
    };
  ...
  return (
    <>
      <WorkspaceNavbar title="DFA Workspace" subtitle="Deterministic Finite Automaton" />
      <div className="pt-24 pb-12 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
              <span className="gradient-text">Deterministic Finite Automaton</span>
            </h1>
            <p className="text-gray-300 text-lg max-w-3xl mx-auto">Create, visualize, and test DFAs with our interactive workspace</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-6">
              <div className="glass-card rounded-2xl p-6 border-2 border-purple-500/30 overflow-hidden relative group">
                ...
                <div className="relative">
                  <input type="text" className="input-field w-full pl-4 pr-12 py-3 rounded-xl text-sm" placeholder="e.g. 'DFA that accepts even number of 0s'" value={aiQuestion} onChange={e => setAiQuestion(e.target.value)} onKeyDown={e => e.key === 'Enter' && askAI()} />
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button className="text-[10px] text-purple-300 hover:text-white transition" onClick={() => setAiQuestion('Contains 101')}>&quot;Contains 101&quot;</button>
                  <button className="text-[10px] text-purple-300 hover:text-white transition" onClick={() => setAiQuestion('Starts with 0')}>&quot;Starts with 0&quot;</button>
                </div>
                {aiLoading && (
              ...
              <div className="glass-card rounded-2xl p-6">
                <h2 className="text-2xl font-bold text-white mb-6 flex items-center">
                  <i className="fas fa-cog text-purple-400 mr-3" /> DFA Configuration
                </h2>
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-gray-300 font-medium mb-2 text-sm">Transitions <span className="text-xs text-gray-500">(one per line: from,symbol,to)</span></label>
                    <textarea className="input-field w-full px-4 py-3 rounded-lg font-mono text-sm resize-none" rows={6} placeholder="q0,0,q1&#10;q0,1,q2&#10;q1,0,q0" value={transitions} onChange={e => setTransitions(e.target.value)} />
                  </div>
                  <div className="flex gap-3">
                    <button className="btn-primary flex-1 px-6 py-3 text-white font-semibold rounded-lg" onClick={generateDFA}>
                      <i className="fas fa-play mr-2" />Generate DFA
                    </button>
                  ...
                  <div className="border-t border-white/10 pt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button className="px-3 py-1.5 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/30 transition text-xs" onClick={() => loadExample('even')}>Binary Even</button>
                      <button className="px-3 py-1.5 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/30 transition text-xs" onClick={() => loadExample('contains101')}>Contains &quot;101&quot;</button>
                      <button className="px-3 py-1.5 bg-purple-500/20 text-purple-300 rounded-lg hover:bg-purple-500/30 transition text-xs" onClick={() => loadExample('div3')}>Divisible by 3</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="glass-card rounded-2xl p-6">
              ...
              <div className="bg-white/5 rounded-xl border border-white/10 relative h-[400px]" ref={containerRef}>
                {!generated && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="text-center">
                      ...
                      <p className="text-gray-400 mb-2">Your DFA will appear here</p>
                      <p className="text-gray-500 text-sm">Configure and click &quot;Generate DFA&quot;</p>
                    </div>
                  </div>
                )}
              </div>
